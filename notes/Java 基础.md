...
理想的散列函数应当具有均匀性，即不相等的实例应当均匀分布到所有可能的散列值上。这就要求散列函数要把所有域的值都考虑进来，可以将每个域都当成 R 进制的某一位，然后组成一个 R 进制的整数。R 一般取 31，原因一般有以下几点：

- 一是因为它是一个奇素数，如果是偶数的话，当出现乘法溢出，信息就会丢失，因为与 2 相乘相当于向左移一位（低位补0）；
- 二是性能较好，即用移位和减法来代替乘法，可以得到更好的性能： 31 * i == (i << 5）- i， 现代的 JVM 会自动完成这种优化；
- 三是若使用较小的质数做为乘子时，冲突率会很高，若取较大的质数作为乘子时，则很容易导致最终的 hash 值溢出（超过 int 类型所能表示的数值大小）。
...
- 浅拷贝：当字段的类型是基本数据类型时，会复制属性及值，当字段的类型是引用类型时，则生成引用指向相同的对象地址。
- 深拷贝：当字段的类型是基本数据类型时，会复制属性及值，当字段的类型是引用类型时，会把当前字段引用的对象再复制一份。
...
使用 clone() 方法来拷贝一个对象即复杂又有风险，它会抛出异常，还需要类型转换。因此不推荐使用 clone()，可以使用拷贝构造函数或者拷贝工厂来拷贝一个对象。
...
1. 提高了程序的灵活性和扩展性，降低耦合性。它允许程序创建和控制任何类的对象，无需提前硬编码目标类。
...
2. 安全限制：部分虚拟机环境对反射有所限制，例如 Android Q 版本开始调用 @hide api 会报错。
...
- 泛型是通过类型擦除来实现的，编译器在编译时擦除了所有类型相关的信息，然后在运行期拿到泛型元数据进行隐式强转，泛型元数据以 Signature 的形式存储在 Class 文件的 Constant pool 中。

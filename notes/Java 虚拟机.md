...
通过 GC Roots 作为起始点进行向下搜索引用的对象，搜索走过的路径称为“引用链”，能够到达到的对象都是存活的，不可达的对象可被回收。

Java 虚拟机使用该算法来判断对象是否可被回收，在 Java 中 GC Roots 一般包含以下内容：

- **运行中的线程**（包括了 Java 虚拟机栈和本地方法栈中引用的对象，例如栈帧中的局部变量表以及在操作数栈中的压栈的对象）；
- static 修饰的对象。

<div align="center"> <img src="../pictures//0635cbe8.png"/> </div><br>

对象 Object5 — Object7 之间虽然彼此还有联系，但是它们到 GC Roots 是不可达的，因此它们 **可能** 会被判定为可回收对象。
...
以 GC ROOT 作为起始点进行向下搜索，让可达的对象（所有存活的对象）都向一端移动，然后直接清理掉端边界以外的内存。
...
使得 Java 类随着它的类加载器一起具有一种带有优先级的层次关系，从而使得基础类得到统一，避免类的重复加载（JVM 区分不同类的方式不仅仅根据类名，相同的类文件被不同的类加载器加载产生的是两个不同的类），也保证了 Java 的核心 API 不被篡改。

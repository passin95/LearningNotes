...
在 JDK1.7 之前运行时常量池是方法区的一部分。JDK1.7 将运行时常量池从方法区中移了出来，在 Java 堆（Heap）中开辟了一块区域存放运行时常量池。从 JDK 1.8 开始作为元空间的一部分移到了虚拟机以外的计算机本地内存。
...
被软引用关联的对象，只有在内存不够的情况下才会被回收（前提是弱引用关联的强引用对象已不在 GC Roots 中）。

...
- 尽量使用 Parcelable 代替 Serializable，前者在性能和速度上都大于后者。同时也可以考虑使用 twitter 开源的序列化框架 [Serial](https://github.com/twitter/Serial/blob/master/README-CHINESE.rst/)，他具备了前面两者的优点。
...
- 对一些觉得非必要的 Crash，在生成环境不做崩溃处理（崩溃的默认处理会杀掉当前进程）。
...
- 如果 RecyclerView 的宽高是不变的，可以 setHasFixedSize(true) 去减少 requestLayout() 操作。

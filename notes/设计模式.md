...
软件设计在某个维度和算法一样，对抗的都是规模问题，只不过，算法对抗的是数据的规模，而软件设计对抗的是需求的规模。

设计模式本质上是前人针对某类特定场景所总结出来的代码设计经验，是软件设计中的一种手段。

## 1.2 作用

用于改善系统的设计，提高代码的复用率、可维护性、可拓展性、可读性，也是架构设计的必备技能之一。
...
- [简单工厂模式](#222-简单工厂模式)：
  1. 产品需要拥有统一的产品接口（也可以是一个抽象类，甚至一个普通的父类，但通常来说接口是最稳定的），对调用方来说，可以屏蔽掉具体产品的入参并且不需要关心具体产品的实现，实现了解耦。
  2. 添加一个 type，对需要代理的方法按按照模块进行业务代码拆分。例如对 RecyclerView.Adapter 的数据处理针按 itemType 进行业务逻辑拆分。
- [工厂方法模式](#223-工厂方法模式)：
...
- [静态代理](#2311-静态代理)：产生一个新的与原有类接口相同却行为不同的类型，往往这些行为是不能或不合适在原有类修改的。
...
- [外观模式](#237-外观模式)：
  ...
  3. 某个方法的执行是需要多个子系统的同时运行，或不同角色在同一个方法中使用不同的子系统。
        ...
        if (sInstance == null) {
            ...
            synchronized (Singleton.class) {
                if (sInstance == null) {
                    ...
                    // ctorInstance(memory);　2：初始化对象（执行 init 构造函数）
...
代理模式可以分为两种，一种是静态代理，一种是动态代理。本质上都是在原有类的行为基础上，加入一些多出的行为，甚至完全替换原有的行为。

#### 2.3.1.1 静态代理

静态代理就是我们手动的将这些行为换进去，产生一个新的与原有类接口相同却行为不同的类型。
   
- 设计原则：遵循依赖导致原则。
- 使用场景：产生一个新的与原有类接口相同却行为不同的类型，往往这些行为是不能或不合适在原有类修改的。

从静态代理的使用上来看，我们一般是这么做的：
  1. 代理类一般都要持有一个被代理的对象的引用；
  2. 对于我们不关心的方法，全部委托给被代理的对象处理，自己只处理我们关心的方法；
  3. 与原有类「接口相同」却行为不同的类型。

- 相关设计模式：
  - 装饰器模式：最大的区别在于装饰器模式使用的是新的类，可以有新的额外方法。

示例：修改 connection.close() 方法由关闭修改为放入连接池中。
...
public class ConnectionProxy implements Connection{
    ...
    public void close() throws SQLException {
        System.out.println("不再真正关闭连接，而是归还给连接池");
    }

...
设计模式只是前人对代码经验的单方面总结，不是用来限制我们敲代码的，不能为了用设计模式而写代码，需要根据实际的团队和需求去考量，因为所编写的代码最终都是给人来阅读和维护的，然后才是顺便给机器运行。
...
- [访问者模式](#248-访问者模式)：将作用于某数据结构中各元素的操作分离出来封装成独立的类，对这些元素添加新的操作，但不改变原数据结构，例如 ASM 中的 classVisitor 中再分别对类注解、变量、方法等进行处理。
- [策略模式](#249-策略模式)：
...
将作用于某数据结构中各元素的操作分离出来封装成独立的类，对这些元素添加新的操作，但不改变原数据结构，实例：asm 中的 classVisitor 中再分别对类注解、变量、方法等进行处理。

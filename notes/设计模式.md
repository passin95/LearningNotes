...
- 行为型：对在不同的对象之间划分责任和算法的抽象化。类的行为模式：使用继承关系在几个类之间分配行为或使用对象聚合的方式分配行为。

24 种设计模式分别为（常用设计模式进行了加粗）：
...
- 设计原则：遵循迪米特、单一职责、开闭原则。
- 使用场景：一个对象有多个维度的变化，需要将这些维度抽离出来，让其独立变化 
  1. 需要拓展第三方 final 类。
  2. 需要装饰的是一个接口、抽象类，并且所装饰的接口、抽象类需要动态去组装或叠加不同的功能。


在软件系统中，某些类型由于自身的逻辑，它具有两个或多个维度的变化，那么如何应对这种“多维度的变化”？如何利用面向对象的技术来使得该类型能够轻松的沿着多个方向进行变化，而又不引入额外的复杂度？这就要使用Bridge模式。而具体使用的方式，则是将抽象部分与他们的实现部分分离，使得它们都可以独立的变化。

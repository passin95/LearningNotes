...
- [二、24 种设计模式](#二24-种设计模式)
    - [2.1 创造型](#21-创造型)
        - [2.1.1 单例模式](#211-单例模式)
        - [2.1.2 简单工厂模式](#212-简单工厂模式)
        - [2.1.3 工厂方法模式](#213-工厂方法模式)
        - [2.1.4 抽象工厂模式](#214-抽象工厂模式)
        - [2.1.5 建造者模式](#215-建造者模式)
        - [2.1.5.1 传统的建造者模式](#2151-传统的建造者模式)
        - [2.1.5.2 笔者常用的建造者模式](#2152-笔者常用的建造者模式)
        - [2.1.6 原型模式](#216-原型模式)
    - [2.2 结构型](#22-结构型)
        - [2.2.1 代理模式](#221-代理模式)
            - [2.2.1.1 静态代理](#2211-静态代理)
            - [2.2.1.2 动态代理](#2212-动态代理)
        - [2.2.2 适配器模式](#222-适配器模式)
        - [2.2.3 装饰器模式](#223-装饰器模式)
        - [2.2.4 桥接模式](#224-桥接模式)
        - [2.2.5 组合模式](#225-组合模式)
        - [2.2.6 享元模式](#226-享元模式)
        - [2.2.7 外观模式](#227-外观模式)
    - [2.3 行为型](#23-行为型)
        - [2.3.1 观察者模式](#231-观察者模式)
        - [2.3.2 模板方法模式](#232-模板方法模式)
        - [2.3.3 命令模式](#233-命令模式)
        - [2.3.4 状态模式](#234-状态模式)
        - [2.3.5 职责链模式](#235-职责链模式)
        - [2.3.6 解释器模式](#236-解释器模式)
        - [2.3.7 中介者模式](#237-中介者模式)
        - [2.3.8 访问者模式](#238-访问者模式)
        - [2.3.9 策略模式](#239-策略模式)
        - [2.3.10 备忘录模式](#2310-备忘录模式)
        - [2.3.11 迭代器模式](#2311-迭代器模式)
...
设计模式只是前人对代码经验的单方面总结，不是用来限制我们敲代码的，不能为了用设计模式而写代码，需要根据实际的团队和需求去考量，因为我们所编写的代码最终都是给人来阅读和维护的，然后才是顺便给机器运行。
...
里式替换原则在大部分情况是需要遵守的，即子类不能删除父类的实现，但规则是死的，有时我们可能出于各种原因，需要违背该原则，因此需要做到灵活使用。
...
- 创建型：对类的实例化进行抽象。
- 结构型：类结构型采用继承机制组合接口实现，对象结构型采用组合对象的方式拓展新功能。
- 行为型：对在不同的对象之间划分责任和算法的抽象化。类的行为模式：使用继承关系在几个类之间分配行为；对象的行为模式：使用对象聚合的方式分配行为。
...
- [单例模式](#211-单例模式)：希望对象在进程中全局单例。
- [简单工厂模式](#212-简单工厂模式)：产品需要拥有 **统一的产品接口** （也可以是一个抽象类，甚至一个普通的父类，但通常来说接口是最稳定的），对调用方来说，可以屏蔽掉具体产品的入参并且不需要关心具体产品的实现，实现了解耦。
- [工厂方法模式](#213-工厂方法模式)：核心关注点更在于是哪个工厂生产的，而不是工厂所生成的产品是什么，工厂在切换生产的产品时，对于客户端来说无需改动任何代码，可维护性更好。
- [抽象工厂模式](#214-抽象工厂模式)：在工厂方法模式的基础上，抽象工厂弥补了工厂方法只能创造一个系列的产品，但需要注意的是对于所有工厂来说是否有 **一致的抽象产品接口**。
- [建造者模式](#2152-笔者常用的建造者模式)：**类的构造函数参数多或复杂**，改造为链式调用变得简洁易读且更易维护（不需要大量重载构造方法）。
- [原型模式](#216-原型模式)：通过拷贝已有的原型对象，创建一个新的对象。
...
- 使用场景：核心关注点更在于是哪个工厂生产的，而不是工厂所生成的产品是什么，工厂在切换生产的产品时，对于客户端来说无需改动任何代码，可维护性更好。
- 相关设计模式：
...
静态代理是我们对方法的实现进行手动替换，而动态代理则更像是一种 AOP 的方式，在调用某个接口类的任意一个方法时，会回调到统一的方法。具体的实战应用和分析可查看 [Retrofit 源码分析](./Retrofit%20源码分析.md#%E4%B8%89retrofitcreate)。
...
- 使用场景：
  1. 需要拓展第三方 final 类。
  2. 需要装饰的是一个接口、抽象类，并且所装饰的接口、抽象类需要动态去组装或叠加不同的功能。

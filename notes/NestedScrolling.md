...
（2）每次子 View 在滑动前都需要将滑动细节传递给父 View，一般情况下在 ACTION_MOVE 事件中调用 dispatchNestedPreScroll()。如果父 View 没有消费完所有距离，view 将自己接着消费并调用 dispatchNestedScroll() 告知 父 View 消费了多少距离。

...
- [触摸反馈](#%E8%A7%A6%E6%91%B8%E5%8F%8D%E9%A6%88)
    - [MotionEvent](#motionevent)
        - [单点触控](#%E5%8D%95%E7%82%B9%E8%A7%A6%E6%8E%A7)
        - [多点触控](#%E5%A4%9A%E7%82%B9%E8%A7%A6%E6%8E%A7)
    - [事件分发机制](#%E4%BA%8B%E4%BB%B6%E5%88%86%E5%8F%91%E6%9C%BA%E5%88%B6)
        - [View 的事件分发机制 dispatchTouchEvent()](#view-%E7%9A%84%E4%BA%8B%E4%BB%B6%E5%88%86%E5%8F%91%E6%9C%BA%E5%88%B6-dispatchtouchevent)
            - [View onTouchEvent()](#view-ontouchevent)
                - [checkForLongClick(0, x, y)](#checkforlongclick0-x-y)
        - [ViewGroup 的 dispatchTouchEvent()](#viewgroup-%E7%9A%84-dispatchtouchevent)
    - [触摸反馈算法](#%E8%A7%A6%E6%91%B8%E5%8F%8D%E9%A6%88%E7%AE%97%E6%B3%95)
    - [手势检测 GestureDetector](#%E6%89%8B%E5%8A%BF%E6%A3%80%E6%B5%8B-gesturedetector)
        - [OnGestureListener](#ongesturelistener)
        - [OnDoubleTapListener](#ondoubletaplistener)
        - [GestureDetector 方法](#gesturedetector-%E6%96%B9%E6%B3%95)
        - [ScaleGestureDecetor 缩放手势](#scalegesturedecetor-%E7%BC%A9%E6%94%BE%E6%89%8B%E5%8A%BF)
            - [ScaleGestureDecetor 方法](#scalegesturedecetor-%E6%96%B9%E6%B3%95)
        - [VelocityTracker](#velocitytracker)
    - [滑动 Scroll](#%E6%BB%91%E5%8A%A8-scroll)
        - [OverScroller](#overscroller)
            - [OverScroller.fling()](#overscrollerfling)
        - [scrollTo()/scrollBy()](#scrolltoscrollby)
    - [拖拽 Drag](#%E6%8B%96%E6%8B%BD-drag)
...
多点触控将 index 属性和事件类型合并以一个 int 作为标识，用最低 8 位 (0x000000ff) 表示事件类型，再往前的 8 位 (0x0000ff00) 表示事件编号。
...
事件分发机制小结：
1. ViewGroup 会从最上层的 View 开始不断向下层 View（ViewGroup） 询问是否接管事件流。
2. onTouchEvent() 和 onInterceptTouchEvent() 都是在 dispatchTouchEvent() 中调用，一个事件分发的过程实质上就是从根 View 递归调用 dispatchTouchEvent() 的过程。
3. 最终根据 View 或 ViewGroup 的 dispatchTouchEvent() 方法的返回值取决于是否消费该事件。

该章节事件分发的源码分析基于 Android SDK 28。
    ...
    if (clickable || (viewFlags & TOOLTIP) == TOOLTIP) {
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                // Drawable 恢复未按压状态、重置各类标记、移除各类 Runnale。
                if (clickable) {
                    setPressed(false);
                }
                removeTapCallback();
...
当 ViewGroup 的 dispatchTouchEvent() 返回 false 时，表示它或它的子 View 不接管事件流。
            ...
            if (actionMasked == MotionEvent.ACTION_DOWN
                ...
                if (newTouchTarget == null && childrenCount != 0) {
                    ...
                    for (int i = childrenCount - 1; i >= 0; i--) {
                        ...
                        // 如果事件的坐标不在子 View 的绘制范围内，
                        // 或 View 不允许接受事件 (即不处于显示状态 (VISIBLE) 或者未播放动画)，
                        // 则跳出语句询问下一个子 View。                      
                        ...
                        // 执行到此，说明该子 View 此前没有被触摸过，
                        // 调用子 View 的 dispatchTouchEvent(event)。
        ...
        if (mFirstTouchTarget == null) {
            ...
            while (target != null) {
                ...
                if (alreadyDispatchedToNewTouchTarget && target == newTouchTarget) { 
                    // 调用到这里说明该次事件为某个子 View 的第一个事件（ACTION_DOWM 或 ACTION_POINTER_DOWN），已在构建该子 View 的 TouchTarget 时便发送到事件到 该 View。
...
GestureDetector 可以根据 MotionEvents 检测各种手势和事件。
...
public interface OnDoubleTapListener {
    ...
    /**
     * 单击时被调用。
     * 在单击事件发生后 300ms 后，且没有后续特殊事件（双击）触发。
     *
     */
...
### ScaleGestureDecetor 缩放手势

ScaleGestureDecetor 有两个监听器：OnScaleGestureListener 和 SimpleOnScaleGestureListener，SimpleOnScaleGestureListener 为 OnScaleGestureListener 的空实现。
...
public interface OnScaleGestureListener {
     ...
     *
     */
    缩放被触发 (会调用 0 次或者多次)，如果返回 true 则表示当前缩放事件已经被处理，检测器会重新积累缩放因子，返回 false 则会继续积累缩放因子。
...
1. 在 ACTION_DOWM 事件中，通过 velocityTracker.obtain() 拿到一个实例（在使用完后释放）；或在初始化的时候实例化，在此时调用 velocityTracker.clear() 进行重置。
...
该方法常用于 OnGestureListener 的 onFling() 方法中来启动惯性滑动的计算：
...
scrollTo()/scrollBy() 多用于滑动控件对绘制时的内容设置偏移。方法参数 scroll 填正值时，绘制内容会负向移动。
...
使用 OnDragListener 不需要自定义 View，调用 startDragAndDrop() 便可开启拖拽，同时会回调到 OnDragListener。
...
- DagEvent.ACTION_DRAG_LOCATION - 当拖动影子图像还在视图的边界框范围中触发。
- DagEvent.ACTION_DROP - 在源视图对象上释放拖动阴影时触发。
- DagEvent.ACTION_DRAG_ENDED - 用户松手事件结束触发。
- DagEvent.ACTION_DRAG_ENTERED - 拖拽影子图像进入某个目标视图时，目标视图会触发该回调。ACTION_DRAG_STARTED 事件中返回 true 的子 View 才能触发。
- DagEvent.ACTION_DRAG_EXITED - 拖动 View 移出了 View 的边界。触发此事件后该事件流不再触发 DagEvent.ACTION_DRAG_LOCATION。
...
- [MotionEvent 详解 - GcsSloop](http://www.gcssloop.com/customview/motionevent)
- [手势检测 - GcsSloop](http://www.gcssloop.com/customview/gestruedector)
...
- [二、事件分发机制](#二事件分发机制)
  ...
  - [2.2 ViewGroup 的事件分发机制](#22-viewgroup-的事件分发机制)
- [三、触摸反馈算法](#三触摸反馈算法)
  - [3.1 基本实现思路](#31-基本实现思路)
  - [3.2 多点触控](#32-多点触控)
    - [3.2.1 多点接力](#321-多点接力)
    - [3.2.2 多点协作](#322-多点协作)
    - [3.2.3 多点互不干扰](#323-多点互不干扰)
- [四、手势检测 GestureDetector](#四手势检测-gesturedetector)
  ...
  - [4.2 ScaleGestureDetector 缩放手势](#42-scalegesturedetector-缩放手势)
    - [4.2.1 ScaleGestureDetector 常用方法](#421-scalegesturedetector-常用方法)
...
public boolean dispatchTouchEvent(MotionEvent ev) {
    ...
    if (onFilterTouchEventForSecurity(ev)) {
        ...
        // 检查事件流是否是ACTION_CANCEL（当前 View 的 父 View 进行了拦截）。
        ...
        if (!canceled && !intercepted) {
            ...
            // 如果是一个按下事件时，需要将该事件分配给一个新的子 View（目的是做到多点触控）。
            if (actionMasked == MotionEvent.ACTION_DOWN
                ...
                if (newTouchTarget == null && childrenCount != 0) {
                    ...
                    for (int i = childrenCount - 1; i >= 0; i--) {
                        ...
                        // 该子 View 已经在屏幕上（在当前事件流处于某一个事件），一般是 MotionEvent.ACTION_HOVER_MOVE。
                        ...
                        if (dispatchTransformedTouchEvent(ev, false, child, idBitsToAssign)) {
                            ...
                            // 添加一个新的 TouchTarget 并添加 TouchTarget 链中，并将它赋值给 mFirstTouchTarget，表示最后一个消费按下事件的子 View。
                ...
                if (newTouchTarget == null && mFirstTouchTarget != null) {
                    // 如果此时没有任何子 View 接受该事件，将该事件自动添加给最后按下的子 View。
        ...
        if (mFirstTouchTarget == null) {
            ...
            // 存在至少一个子 View 正在消费该事件流。
            
            TouchTarget predecessor = null;
            // 遍历 TouchTarget 链，也就是遍历当前还在消费事件的子 View（就是手指还没抬起）。
            ...
            while (target != null) {
                ...
                if (alreadyDispatchedToNewTouchTarget && target == newTouchTarget) { 
                    ...
                    // 调用到这里说明该次事件非 ACTION_DOWM 或 ACTION_POINTER_DOWN 或者新的多次按下作用于同一个子 View。
        ...
        if (canceled
            ...
            // 如果是 ACTION_POINTER_UP 事件（有一个手指抬起），将该 PointerId 从 TouchTarget 链中移除。
            final int actionIndex = ev.getActionIndex();
...
该种方式用于只针对单点触摸算法的编写，但又需要兼容多点触摸的场景，达到触摸点接力的效果。

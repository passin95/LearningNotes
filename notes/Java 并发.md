...
- [三、Java 内存模型](#三java-内存模型)
    - [3.1 CPU Cache 模型](#31-cpu-cache-模型)
        - [3.1.1 基本模型](#311-基本模型)
        - [3.1.2 硬件层面优化](#312-硬件层面优化)
    - [3.2 重排序](#32-重排序)
    - [3.3 内存屏障](#33-内存屏障)
        - [3.3.1 CPU 层面的内存屏障](#331-cpu-层面的内存屏障)
        - [3.3.2 Java 内存模型层面的内存屏障](#332-java-内存模型层面的内存屏障)
    - [3.4 Java 内存模型](#34-java-内存模型)
        - [3.4.1 内存间交互操作](#341-内存间交互操作)
    - [3.5 内存模型三大特性](#35-内存模型三大特性)
        - [3.5.1 原子性](#351-原子性)
        - [3.5.2 可见性](#352-可见性)
        - [3.5.3 有序性](#353-有序性)
...
<div align="center"> <img src="../pictures//CPU缓存模型_基本.webp"/></div>
...
当 CPU 需要的数据在其他 CPU Cache 内时，需要请求并且等待响应，这显然是一个同步行为。

因此可以采用异步的方案进行优化：在 CPU 和 Cache 之间加一个 store buffer：

1. CPU 可以先将数据写到 store buffer，同时给其他 CPU 发送消息，然后继续做其它事情；
2. 等到收到其它 CPU 发过来的响应消息，再将数据从 store buffer 移到 cache line。

<div align="center"> <img src="../pictures//CPU缓存模型_StoreBuffer.webp"/></div>

但会导致同一个 CPU 存在对数据的两份拷贝，一份在 Cache，一份在 store buffer，而 CPU 取的是 Cache 的数据进行计算，而正确的数据还没有从 store buffer 的值同步到 Cache 中。

（2）Store Forwarding

因此在 store buffer 的基础上，实现了 store forwarding 技术: CPU 可以直接从 store buffer 中加载数据。

<div align="center"> <img src="../pictures//CPU缓存模型_StoreForwarding.webp"/></div>

虽然现在解决了同一个 CPU 读写数据的问题，但多个 CPU 之间工作依旧存在问题。

（3）Invalid Queue

出现内存屏障时，后续的所有写入操作都会挤压在 store buffer 中，导致 store buffer 很容易会满，当 store buffer 满了之后，CPU 还是会卡在同步等待对应的 Invalidate ACK 以处理 store buffer 中的条目。

Invalidate ACK 耗时的主要原因是 CPU 要先将对应的 cache line 置为 Invalid 后再返回 Invalidate ACK，一个很忙的 CPU 可能会导致其它 CPU 都在等它回 Invalidate ACK。

解决思路还是同步化为异步：CPU 先不处理 cache line，而是可以先将 Invalid 消息放到某个请求队列 Invalid Queue，然后就立刻返回 Invalidate ACK，之后再慢慢处理 Invalid Queue 中的消息（也就是处理 cache line）。

<div align="center"> <img src="../pictures//CPU缓存模型_InvalidQueue.webp"/></div>

## 3.2 重排序

为了提高程序运行的性能，编译器和处理器常常会对指令做重排序，具体可分成两类：

- 编译器优化的重排序：编译器在不改变「单线程」程序语义的前提下，可以重新安排语句的执行顺序；
- 处理器重排序：为了异步化指令的执行，引入 Store Buffer 和 Invalidate Queue，却导致了指令顺序改变。

这些重排序在多线程情况下，可能会出现内存可见性、有序性问题。那么如何处理呢？

- 对于编译器优化的重排序：禁止特定类型的编译器重排序；
- 对于处理器重排序：要求编译器在生成指令序列时，插入特定类型的「内存屏障指令」，通过内存屏障指令来禁止特定类型的处理器重排序。

## 3.3 内存屏障

从上文可知，CPU 为了优化指令的执行效率，引入了 store buffer（forwarding），也因此导致了指令重排序，要保证这种顺序一致性，硬件层面优化不了了，需要在软件层面支持，因此引入了「内存屏障」。它是硬件之上，操作系统之下对一致性的最后一层保障。

内存屏障（Memory Barrier），也称为内存栅障，屏障指令等，是一类同步屏障指令，是 CPU 或编译器在对内存随机访问的操作中的一个同步点，**使得此点之前的所有读写操作都执行后才可以执行此点之后的操作**。

### 3.3.1 CPU 层面的内存屏障

CPU 层面的内存屏障分为三类：

- 写屏障（Store Memory Barrier）：告诉处理器将写屏障之前的存储在存储缓存(store buffere)中的数据同步到主内存，简单来说就是使得写屏障之前的指令的结果对写屏障之后的读或者写是可见的；
...
### 3.3.2 Java 内存模型层面的内存屏障

Java 内存模型将内存屏障分为以下四类：

<div align="center"> <img src="../pictures//JMM内存屏障.webp" /> </div>

## 3.4 Java 内存模型

Java 内存模型指定了 Java 虚拟机在计算机的软硬件上的工作方式。Java 内存模型只是一个抽象的概念，但 JVM 应该遵守这些概念去设计：

- 每个线程都有自己私有的工作内存，也称为线程本地内存，主要就是「JVM 运行时数据区域」中的「程序计数器」、「Java 虚拟机栈」和「本地方法栈」。
...
volatile 本质上是使用了汇编指令 **lock**，相当于加了一个 CPU 层面的「全屏障」，在 JVM 层面相当于加入了 JVM 内存屏障。即为内存的执行提供以下几个保障：

- 确保执行到内存屏障修饰的指令时，前面的代码全部执行完成；
- 确保指令重排序时不会将后面的代码排到内存屏障之前；
- 确保指令重排序时不会将前面的代码排到内存屏障之后；
- 禁掉缓存优化，直接使用缓存一致性协议（一般为 MESI 协议）保证 CPU 缓存一致性问题。

从 volatile 的原理简单总结 volatile 的作用：

- 禁止指令的重排序优化（内存屏障）；
- volatile 只能保持变量的可见性（缓存一致性协议），不能保证变量的操作都是原子操作.
...
在多线程环境下，存在某⼀个线程对资源进⾏写操作的中途（写⼊已经开始，但还没结束），其他线程对这个写了⼀半的资源进⾏了读操作，或者基于这个写了⼀半的资源再进⾏写操作，导致出现数据错误。
...
AQS 内部维护一个 state 状态位，尝试加锁的时候通过 CAS + volatile 修改值，如果成功设置为 1，并且把当前线程 ID 赋值，则代表加锁成功，一旦获取到锁，其他的线程将会进入阻塞队列自旋阻塞，获得锁的线程在释放锁的时候将会唤醒阻塞队列中的线程，并把 state 重新置为 0，同时当前线程 ID 置为空。
...
static {
    try {
        ...
        // unsafe 通过直接操纵虚拟机的内存地址来拿到对象。
        // valueOffset 为拿到对象所处的内存偏移地址。
...
- Ncpu：处理器的核的数量，可以通过 Runtime.getRuntime().availableProcessors() 得到；
- Ucpu：期望的 CPU 利用率（该值应该介于 0 和 1 之间）；
- W/C：等待时长与计算时间的比率。

简单的说，需要根据实际程序中，CPU 在运行过程中计算时长和等待时长（例如请求或 IO 操作）各占的比值去考量合适的线程数。

假定 CPU 个数为 1，期望 CPU 利用率为 1，等待时长和计算时长各为 50%，则 Nthreads = 1 * 1 * (1 + 0.5/0.5) = 2 个线程。
